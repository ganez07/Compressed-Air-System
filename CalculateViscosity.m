function [ nu ] = CalculateViscosity( pressure,Temperature )
%UNTITLED Calculates the kinematic viscosity
%   Detailed explanation goes here -> nachträglich noch erledigten
%   Input: pressure [Pa], temperature [K]
%   Output: kinematic viscosity [m^2/s]*10^-6

p = pressure/(10^5); % translation from pascal to bar 
T = Temperature-273.15; % translation from Kelvin to Celsius 

% Initial vectors for formular-calculatins
nu = zeros(size(p));
p_c = zeros(size(p));
p_c(1:size(p)) = 39.5; % kritischer Druck in bar 
T_c = zeros(size(p)); 
T_c(1:size(p)) = 133; % kritische Temperatur in Kelvin
M = zeros(size(p));
M(1:size(p)) = 28.949; % Molare Masse in g/mol
...
for i = 1:size(p)
    if p(i)>= 1 && p(i)< 50
        ...
        % viscosity at 1 bar in 10^-7 Pas at certain Temperature
        ...
        % Calculation of the density at Point X(p,T) in Kg/m^3
        rho_x(i) = (p(i)*10^5)/(1000*(8.3143/28.949)*(T(i)+273.15)); 

        % Calculation of p_r 
        p_r(i) = rho_x(i)/313; % p_r = rho_x geteilt durch Dichte an krit. Punkt [Kg/m^3]
